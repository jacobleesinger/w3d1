...
def cl_to_lr
  ...
  JOIN
    stops AS lr ON lr.id = b.stop_id
  WHERE
    a.stop_id = 53 AND lr.name = 'London Road'
    -- (
    --   SELECT
    --     stops.id
    --   FROM
...
def craiglockhart_and_tollcross
  ...
  JOIN
    routes b ON a.company = b.company AND a.num = b.num
  JOIN
    stops cl ON cl.id = a.stop_id
  JOIN
    stops tc ON tc.id = b.stop_id
  WHERE
    cl.name = 'Craiglockhart' AND tc.name = 'Tollcross'
...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
  SELECT DISTINCT
    end_route_stop.name, end_route.company, end_route.num
  FROM
    routes AS start_route
  JOIN
    routes AS end_route ON start_route.num = end_route.num
     AND start_route.company = end_route.company
  JOIN
    stops AS end_route_stop ON end_route_stop.id = end_route.stop_id
  JOIN
    stops AS start_route_stop ON start_route_stop.id = start_route.stop_id
  WHERE
    start_route_stop.name = 'Craiglockhart'
...
def craiglockhart_to_sighthill
  ...
  FROM
    stops AS start_stop
  JOIN
    routes AS start ON start.stop_id = start_stop.id
  JOIN
    routes AS to_transfer ON start.num = to_transfer.num
  AND
  ...
  JOIN
    routes as finish ON finish.num = from_transfer.num
  AND
    finish.company = from_transfer.company
  JOIN
    stops AS final_stop ON final_stop.id = finish.stop_id
  WHERE
